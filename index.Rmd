...
In this study, I’m interested in exploring the extent to which public transport has been impacted by the COVID-19 pandemic. More specifically, which form of public transport has been hit the hardest? I plan to examine data from the New York Metropolitan Transportation Authority (MTA), containing statistics on ridership and the corresponding proportion of pre-pandemic ridership levels for each of the forms of public transport offered under the MTA in the greater New York City metropolitan area. I hypothesize that the Subway form of public transport suffered the largest impact on ridership from the pandemic, due to it having the highest ridership levels relative to all other forms of public transport, as well as the unavoidable proximity riders experience, and as a consequence, is likely viewed to be of highest-risk.

## Data
The MTA is a public benefit corporation responsible for public transportation in the state of New York. This dataset provides systemwide ridership and traffic estimates for subways (including the Staten Island Railway), buses, Long Island Rail Road, Metro-North Railroad, Access-A-Ride, and Bridges and Tunnels, beginning March 1st, 2020 (April 1st, 2020 for LIRR and Metro-North), and provides a percentage comparison against a comparable pre-pandemic date (the equivalent month in 2019). Ridership figures (and subsequently, the `% of Comparable Pre-Pandemic Day` values [the dependent variable in this study]) are determined through what can be generalized as a census method -- a combination of MetroCard, OMNY swipes/taps, ticket sales, scheduled trips, and toll collection systems -- tracked over time (the independent variable) on a daily basis from March 1st, 2020 to present.

The research design for this study follows the differences-in-differences method, with the "treatment" being the pandemic, the "treatment group" being the ridership levels pre-pandemic (2019 data), and the "control group" being the ridership levels after the onset of the pandemic (post-March 1st, 2020). Through the differences-in-differences research design, we are able to measure the effect the "treatment" (the pandemic) had on ridership over time. Below is a boxplot summarizing the `% of Comparable Pre-Pandemic Day` values for each type of transportation. As seen in the boxplot, the median `% of Comparable Pre-Pandemic Day` value hovers around 50-60% for most forms of transport, with some notably high exceptions for Bridges and Tunnels and Access-a-Ride.
...
### % of Comparable Pre-Pandemic Day Values, by type
``` {r}
ridership_prop |>
...
### % of Comparable Pre-Pandemic Day Values, over time
```{r}
ridership_prop |>
  group_by(type) |>
  ggplot() + 
